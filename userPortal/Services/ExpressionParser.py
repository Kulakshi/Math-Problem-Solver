...
class ExpressionParser(object):
    ...
    def parse_prefix_2(self, string, explicit=False):
        ...
        word = Word(alphas)
        operator = Word('∪∩', max=1)
        element = integer | letter | word
        set_name = OneOrMore(word) | letter
        set_complement = set_name + '\''
        set = set_name | set_complement
        expression = set + operator + set | '(' + set + operator + set + ')' | set | '(' + set + ')'
        card = 'n' + expression | ('N' + expression) | ('|' + expression + '|')
        elem_list = element + ZeroOrMore(',' + element)
        list_def = '{' + elem_list + '}'
        eqn =  card + '=' + integer | expression + '=' + list_def |card + '=' + card

        parsed = eqn.parseString(string)
        print("parsed", parsed)
    ...
    def get_parser(stack, function_indices):
        ...
        operator = Word('∪∩', max=1)
        element = integer | letter |  word
        set_name = word | letter
        set_complement = (set_name + '\'').setParseAction(_push_func)
        set = (set_name | set_complement).setParseAction(_push)
        expression = set + operator + set | Literal('(').suppress() +set + operator + set + Literal(')').suppress()
        card = ('n' + expression).setParseAction(_push) #| ('N' + expression) | ('|' + expression + '|')
        elem_list = element + ',' + element
        list_def = Literal('{').suppress() + elem_list + Literal('}').suppress()
        eqn = (card + '=' + card | card + '=' + integer | expression + '=' + list_def).setParseAction(_push)
