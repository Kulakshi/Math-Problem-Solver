...
class info:
    ...
    def check_and_store(self, index, region):
        ...
        region.expr = util.get_expr_for_region(index, pow(2, self.num_sets))
        if(region.size is not None):
    ...
    def validate_size(self, region_index, size):
        super_sets = util.get_super_sets(region_index, pow(2, self.num_sets))
        if size is not None:
            ...
            sub_sets = util.get_sub_sets(region_index, pow(2, self.num_sets))
    ...
    def get_region_index(self, region_name):
        region_name = util.convert_region_name_to_logical_exp(region_name)
        ...
        if region_name == 'ξ':
            ...
            return util.evaluate_logic_exp_tree(exp, self.num_sets, self.main_name_index_map, self.get_union_index,
